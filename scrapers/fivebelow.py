...
class FiveBelowSearcher:
    """Five Below product search via Instacart's GraphQL search endpoint —
    the same API the other storefront scrapers use (same response shape),
    just on instacart.com with Five Below's shop/zone ids."""
    ...
    def warmup(self, max_retries: int = 3):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                r = self.session.get(
                    "https://www.instacart.com/store/five-below",
    ...
    def _search_once(self, query):
        ...
        variables = {
            "action": None,
            "query": query,
            "pageViewId": "be014120-f14f-5367-9f36-bd0e87fd4654",
        ...
        headers = {
            ...
            "referer": f"https://www.instacart.com/store/five-below/s?k={quote(query)}",
            "x-client-identifier": "web",
            "x-client-user-id": "21192267958514052",
            "x-ic-view-layer": "true",
            "x-page-view-id": "be014120-f14f-5367-9f36-bd0e87fd4654",
        }
        ...
        # with open(Path(__file__).parent / "fivebelow_last_response.json", "w", encoding="utf-8") as f:
        #     json.dump(data, f, indent=2, ensure_ascii=False)
        # print("✓ Raw response saved to fivebelow_last_response.json")
        ...
        return {
            ...
            "product_url": f"https://www.instacart.com/store/five-below/products/{evergreen_url}",
    ...
    def _get_description_once(self, product_id):
        ...
        url = f"https://www.instacart.com/store/five-below/products/{product_id}"
...
if __name__ == "__main__":
    ...
    five_below = FiveBelowSearcher(proxy)

    five_below.warmup()

    product = five_below.search("Amos Peelerz Mummies")
