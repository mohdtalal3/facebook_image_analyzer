...
def download_image(url, post_id, image_index=1, save_dir="page_post", max_retries=3):
...
def extract_creation_time(node):
    """Best-effort extraction of the post's creation time (unix seconds) from
    the story node. Facebook's GraphQL schema doesn't expose this at a fixed
    path we can rely on, so this scans for the first 'creation_time' field
    anywhere in the node. Returns None if not found."""
...
def extract_media(node, post_id, save_dir="page_post", max_images=None, download_images=True,
    ...
    def _cap_reached():
        return max_images is not None and _image_counters[post_id] >= max_images
    ...
    for att in attachments:
        ...
        if single_media:
            # Single photo case
            if "photo_image" in single_media and not _cap_reached():
            ...
            elif "image" in single_media and not _cap_reached():
                _image_counters[post_id] += 1
                last_media_id = single_media.get("id")  # Track the last media ID
...
def fetch_posts(limit=10, min_comments=0, batch_size=10, on_batch_complete=None,
    ...
    while len(all_posts) < limit and page_num <= max_pages:
        ...
        for node in story_nodes:
            ...
            creation_ts = extract_creation_time(node)
            published_at = None
            if creation_ts:
            ...
            # Extract media with correct save directory. Per-post failure
            # isolation: one malformed node crashing extract_media() must not
            # kill the whole source scrape — the post is skipped, pagination
            # continues.
            try:
                ...
                print(f"  ⚠️ Failed to extract/save post {post_id}, skipping: {e}")
                continue
